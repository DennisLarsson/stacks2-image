...
def run_denovo_map_nm(n_val, m_val, cpu_count, popmap_path):
...
def run_optimization(cpu, args, val_m=None):
    ...
    for val in range(args.min_val, args.max_val + 1):
        if val_m is None:
            R80_val = run_denovo_map_nm(val, val, cpu, args.popmap)
        else:
            if val == val_m:
                continue
            R80_val = run_denovo_map_nm(val, val_m, cpu, args.popmap)
